...
class Trainer(AbstractTrainer):
    ...
    def _save_embeddings_if_applicable(self, checkpoint_file, verbose=True):
        r"""Save final embeddings from graph-based models (LightGCN, NGCF, etc.)
        
        For models that have a forward() method returning (user_emb, item_emb),
        extract and save the final aggregated embeddings alongside the checkpoint.
        
        Args:
            checkpoint_file (str): path to the saved checkpoint
            verbose (bool): whether to log the save operation
        """
        import numpy as np
        
        # Only save embeddings for models that support it
        if not hasattr(self.model, 'forward'):
            return
        
        # Check if this is a graph-based model (has embedding tables)
        if not (hasattr(self.model, 'user_embedding') and hasattr(self.model, 'item_embedding')):
            return
        ...
        try:
            with torch.no_grad():
                ...
                item_emb_np = item_emb.cpu().float().numpy()
                
                # Construct embedding file path (replace .pth with -embeddings.npz)
                embedding_file = checkpoint_file.replace('.pth', '-embeddings.npz')
                
                # Save both embeddings in a single compressed file
                np.savez_compressed(
                    embedding_file,
                    user_embeddings=user_emb_np,
                    item_embeddings=item_emb_np,
                )
                
                if verbose:
                    self.logger.info(
                        set_color("Saving embeddings", "blue") 
                        + f": {embedding_file} "
                        + f"(users: {user_emb_np.shape}, items: {item_emb_np.shape})"
                    )
        except Exception as e:
            # Silently fail if embeddings cannot be saved
            # (model might not support it or might be on distributed setup)
            if verbose:
                self.logger.warning(
                    f"Could not save embeddings: {str(e)}"
                )
    ...
    def _save_checkpoint(self, epoch, verbose=True, **kwargs):
        ...
        # ── NEW: Save final embeddings for graph-based models (LightGCN, etc.) ────
        self._save_embeddings_if_applicable(saved_model_file, verbose)
